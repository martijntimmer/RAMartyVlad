...
specularIntensity = pow(max(dot(N, normalize(V+L)),0),mat.shininess); // compute specular intensity
...
if (specular) { result += specularIntensity * mat.specular * light.specular; } // compute and add specular contribution
    return result;
}
